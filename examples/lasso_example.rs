...
use coal_mind_linear_regression::lasso;
use nalgebra as na;
...
const NUM_FEATURES: usize = 3; // We'll use 3 features to demonstrate feature selection
const OUTPUT_FILE: &str = "examples/output/lasso_chart.png";

/// Generates synthetic data with multiple features, where only some are truly relevant
fn generate_sample_data() -> (na::DMatrix<f64>, na::DVector<f64>) {
    ...
    for i in 0..NUM_POINTS {
        // First feature: meaningful relationship
        let x1 = i as f64 * 10.0 / NUM_POINTS as f64;
        x1_values.push(x1);
        features.push(x1);

        // Second feature: weak relationship with noise
        features.push(x1 * 0.2 + normal.sample(&mut rng));

        // Third feature: pure noise (should be eliminated by Lasso)
        features.push(normal.sample(&mut rng));
    }

    // True relationship: y = 2x₁ + 0.2x₂ + noise
    // x₃ is irrelevant and should get zero coefficient with sufficient regularization
    let y_values: Vec<f64> = (0..NUM_POINTS)
        .map(|i| {
            2.0 * features[i * NUM_FEATURES]
                + 0.2 * features[i * NUM_FEATURES + 1]
...
/// Creates plots comparing different Lasso regularization strengths
fn plot_lasso_comparison(
    ...
    // Left panel: Scatter plot with regression lines for feature 1
    {
        ...
        let mut chart = ChartBuilder::on(&left_panel)
            .caption("Lasso Regression: Feature 1 vs Target", ("sans-serif", 20))
            .margin(5)
    ...
    // Right panel: Coefficient paths
    {
        let mut chart = ChartBuilder::on(&right_panel)
            .caption("Lasso Coefficient Paths", ("sans-serif", 20))
            .margin(5)
            .x_label_area_size(30)
            .y_label_area_size(30)
            .build_cartesian_2d(
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &alpha in &alpha_values {
        let model = if alpha == 0.0 {
            ...
            lasso::fit(&features, &targets, alpha, 1000, 1e-4)
        }
    ...
    plot_lasso_comparison(&features, &targets, &models)?;
